...
function toList(value) {
  ...
  if (Array.isArray(value)) return value.flatMap((item) => typeof item === 'string' ? [item] : [normalizeText(item)]).filter(Boolean);
  if (typeof value === 'object') return Object.entries(value).map(([key, item]) => `${key}: ${normalizeText(item)}`);
  return [String(value)];
}

function isColorLike(value) {
  return typeof value === 'string' && (/^#([0-9a-f]{3}){1,2}$/i.test(value.trim()) || /^rgb/.test(value.trim()));
}
...
function makeColorChips(value) {
  ...
  toList(value).forEach((item) => {
    ...
    const color = label.match(/#([0-9a-f]{3}){1,2}\b/i)?.[0] || (isColorLike(label.trim()) ? label.trim() : '');
...
function renderResult(payload) {
  ...
  const summary = findFirst(data, ['summary', 'zusammenfassung', 'analysis', 'analyse', 'description', 'beschreibung', 'result']);
  ...
  const colors = findFirst(data, ['colors', 'colours', 'palette', 'farben', 'colorPalette', 'farbpalette']);
  ...
  if (recommendations) grid.append(makeCard('Empfehlungen', toList(recommendations), { full: true }));
