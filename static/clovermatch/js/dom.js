...
export function updateStats() {
  ...
  const pct = gp.dual
    ? (gp.streakOk ? 100 : Math.min(100, (game.score / lv.goal.score) * 100))
  ...
  const recommendation = Math.min(100, Math.round(game.clarity * 0.72 + game.counts.letgo * 7 + game.streak * 1.5));
  $('barA').style.width = analysis + '%';
  $('barB').style.width = recommendation + '%';
  $('aPct').textContent = analysis + '%';
  $('bPct').textContent = recommendation + '%';
  const balanced = [game.counts.color, game.counts.form, game.counts.impact, game.counts.life].filter((v) => v > 0).length;
