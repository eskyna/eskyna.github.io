---
title: "Что означает осенний цветотип?"
term: "Осенний цветотип"
description: "Осенний цветотип это тёплый, чаще приглушённый тип внешности с землистыми, насыщенными и природными оттенками."
category: "Консультация по цвету и цвет"
tags: ["осенний цветотип", "цветотип", "цветовая консультация", "autumn type"]
keywords: ["осенний цветотип", "soft autumn", "warm autumn", "deep autumn", "тёплый осенний тип"]
glossar_order: "h"
weight: 9
relatedTerms:
  - title: "Цветотип"
    url: "/ru/glossar/farbtyp/"
  - title: "Подтон"
    url: "/ru/glossar/unterton/"
  - title: "Приглушённые цвета"
    url: "/ru/glossar/gedaempfte-farben/"
  - title: "Цветовая температура"
    url: "/ru/glossar/farbtemperatur/"
  - title: "Цветовой паспорт"
    url: "/ru/glossar/farbpass/"
---
