---
title: "Что означает полиэстер?"
term: "Полиэстер"
description: "Полиэстер это синтетическое волокно, которое может быть простым в уходе и в зависимости от качества выглядит очень по-разному."
category: "Материалы и детали"
tags: ["материалы", "полиэстер"]
keywords: ["полиэстер", "свойства полиэстера", "полиэстер в одежде"]
weight: 1
image: "images/glossar/polyester.png"
image_alt: "Иллюстрация глоссария ESKYNA для термина Полиэстер"
relatedTerms:
  - title: "Пластика материала"
    url: "/ru/glossar/materialfall/"
  - title: "Фактура"
    url: "/ru/glossar/struktur/"
  - title: "Степень блеска"
    url: "/ru/glossar/glanzgrad/"
  - title: "Аксессуар"
    url: "/ru/glossar/accessoire/"
---
