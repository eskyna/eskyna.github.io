---
title: "Что означает трикотаж?"
term: "Трикотаж"
description: "Трикотаж это вязаное полотно, которое в зависимости от пряжи, плотности и поверхности может выглядеть мягко, тонко, фактурно или объёмно."
category: "Материалы и детали"
tags: ["материалы", "трикотаж"]
keywords: ["трикотаж", "что означает трикотаж", "трикотаж в стилизации"]
weight: 1
image: "images/glossar/strick.png"
image_alt: "Иллюстрация глоссария ESKYNA для термина Трикотаж"
relatedTerms:
  - title: "Пластика материала"
    url: "/ru/glossar/materialfall/"
  - title: "Фактура"
    url: "/ru/glossar/struktur/"
  - title: "Степень блеска"
    url: "/ru/glossar/glanzgrad/"
  - title: "Аксессуар"
    url: "/ru/glossar/accessoire/"
---
